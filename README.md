...
### File System :briefcase:
> * :crystal_ball: [Teleport](#crown-place-it-a-top-my-own-hand-crown) :milky_way:
```
    > หน้า(01-10)
        ในช่วงแรกเราจะบอกว่า File system เอาเขาต้องการอะไรบ้าง ซึ่งสิ่งที่เขาต้องการก็จะมี คุณสมบัติของ Long Term Storage
            1. เขาอยากอยากได้พื้นที่ใหญ่ๆ ในการเก็บข้อมูล
            2. อยากให้ไฟล์ที่มีการใช้เสร็จแล้วอะ สามารถใช้ซ้ำได้อีกในรอบถัดๆไป ที่มีการเรียกขึ้นมาใช้
            3. อยากให้ไฟล์มีคุณสมบัติเป็น Sharing file
        แล้วก็สิ่งที่เป็นข้อบังคับ คือ 
            1. OS จะรู้ได้ไงว่า ไฟล์นี้ถูกเก็บอยู่ที่ไหน
            2. ถ้าหากคอมมีระบบ Multiple User อะ เราจะสามารถรักษาสิทธิ์การเข้าถึงของไฟล์นี้ได้อย่างไร
            3. OS จะรู้ได้ไงว่า พื้นที่ไหนว่างที่จะเก็บไฟล์นี้
        - File Name(File Extensin-นามสกุลของไฟล์) : มีหน้าที่ในการช่วยให้ OS รู้ว่าเมื่อ User double-click ที่ไฟล์นั้นแล้ว Os จะไปเรียก Program ไหนที่เหมาะสมที่จะเปิดไฟล์นั้นได้
        - File Structure : มี 3 ประเภท ได้แก่
            - Byte Sequence : คือ Byte Sequence จะมี File pointer เพื่อชี้ไปยังตำแหน่งของ Sequnce นั้นๆ เช่น ถ้าหากว่าไฟล์นั้นมีการอ่าน Integer File pointer ก็จะเลื่อนไป 4 bytes เพื่ออ่านตัวเลขขึ้นมา แล้วถ้าหากว่าในภายหลังเกิดการเขียน char 1 ตัว File pointer ก็จะเลื่อนไปอีก 1 ช่อง ที่นี้ถ้าหากว่าเราต้องการที่จะแก้ byte ตอนแรกอะ มันก็ทำไม่ได้และเพราะว่า ตอนนี้ File pointer ชี้มา ช่องที่ 5 แล้วอะ ฉะนั้นเราเลยเกิด ฟังก์ชัน FSEEK ขึ้นมาเพื่อช่วยในการเลื่อนหา byte ที่ต้องการจะแก้
            - Record Sequence : Record ก็เหมือนกันแต่ว่าใน Record อะงับเขาจะมีการเก็บ Attr. ของไฟล์เอาไว้ด้วยซึ่งเขาเรียกว่ามันคือ Metadata แต่ว่าการเข้าถึง Record ก็ยังเข้าถึงแบบ sequence อยู่ดี
        **Metadata คือ คุณลักษณะของไฟล์ที่แบบมีการบอกว่า เออไฟลนี้ชื่ออะไร มีขนาดเท่าไร ใครมีสิทธิ์เข้าถึงได้บ้างอะไรประมาณนี้**
        ซึ่งอย่างที่รู้ๆกันว่า การเข้าถึงไฟล์แบบ Sequence มันนานนะถ้ามีหลายๆไฟล์ ดังนั้นเลยเกิด Structure แบบใหม่คือ 
            - Tree use by key : อันนี้แหละงับ น้อนคือการใช้ key ในการเข้าถึงไฟล์นั้นๆเพื่อเปลี่ยนแปลง ดูหน้า(5)
        - File Type : คือลักษณะของไฟล์ ซึ่งไฟล์แบ่งเป็น 2 ส่วน คือ
            1. Header : คือส่วนที่บ่งบอกถึง Metadata ซึ่งก็คือ File type นั้นเองที่จะมีการเก็บ File Attr.
            2. Object Modular : คือ Data content งับจบ
        - File Operation : คือการเรียกใช้ System call ในการทำงานกับ file เช่น
            - create/Delete
            - Read/Write
            - Open/Close
            และอื่นๆ
    > หน้า(11-19)
        คือ เมื่อก่อนอะไฟล์เราเก็บใน Root Directory เดียว แล้วข้างใน Directory(Folder) นั้นอะก็มีอีกหลายไฟล์ที่เก็บไว้ข้างในนั้น ซึ่งพอเวลาผ่านไปนานๆ ไฟล์ก็เริ่มเยอะขึ้น เวลาเราจะเอาหาไฟล์สักไฟล์แม่งก็นานขึ้น ทีนี้เลยทำให้เราเกิดระบบการจัดเก็บไฟล์แบบใหม่ เราเรียกว่า Hierarchical Directory Systems คือการเก็บ ไฟล์แบบเป็นขั้นๆ 
        - Path Name : คือที่มาของไฟลนั้นๆก่อนจะมีถึงไฟล์ ณ ปัจจุบันที่เรากำลังอยู่ ซึ่งสามารถแบ่งออกได้เป็น 2 path นั่นก็คือ
            Absolute Path : หมายความว่าเราจะเอาตั้งแต่ Root->Current Folder
            Relative Path : หมายความว่าเราจะเอาแค่ file ที่อยู่ใน folder นั้น
            Ex. current folder = usr
            Ab : /usr/jim/a
            Re : /jim/a
            ดูจากหน้า(13) นะ
        - การจัดสรรพื้นที่ให้กับ File ต่างๆ มี 3 วิธีได้แก่ 
            1. Contiguous Allocation : คือหลังการเดิมคับ เอาเข้ามาทั้งไฟล์และทุกไฟล์ต้องติดกันแบบ sequential ซึ่งถ้าหากมีบางไฟล์โดนลบ ก็จะทำให้พื้นที่นั้นกลายเป็นพื้นที่ว่างสร้าง External Fragmentation ซึ่งเราไม่ต้องการ ดังนั้นการที่ OS จะมีวิธีที่ดีกว่าการทำ sequential คือการที่ จะต้องมีการเก็บ Start Block และ Size ของ Block เอาไว้เพื่อให้ OS ทำงานได้ง่ายขึ้นนั้นเอง เพราะว่าถ้าหาก Block ไหนโดนลบก็จะไม่มีการเก็บ start และ size ของ Block นั้นเอาไว้
            2. Linked List Allocation : คือการเก็บแบบใช้ Linked List ซึ่งแน่นอนว่า 1 block disk ของเราจะต้องแบ่งพื้นที่ส่วนนึงไปเก็บ pointer เพื่อชี้ไปยัง block ถัดไปเพื่อไปเอา content จนถึง block สุดท้าย นอกจากนี้การใช้ Linked List ใช้ช่วยให้เราไม่ต้องเก็บไฟล์แบบติดกันอีกด้วย หรือที่เรียกว่า Non-Contiguous Allocation นั่นเอง
            3. Linked List Table : คือการที่สร้าง File Allocation Table เราไม่จำเป็นต้องแบ่งพื้นที่ของ Block Disk เพื่อเก็บค่า Pointer เพื่อชี้ไปยัง Block ถัดไปอีกแล้ว เราสามารถใช้ตัวเลขที่อยู่ใน Block Disk เพื่อ Link Index ภายใน FAT ได้เลย ทำให้ Block Disk สามารถเก็บ content ได้เยอะขึ้นนั่นเอง
        - วิธีการจัดการเก็บชื่อไฟล์ : การเก็บชื่อไฟล์นั้นก็เป็นส่วนนึงของสิ่งที่ต้องเก็บในไฟล์เหมือนกันแต่ว่า เราควรจะเก็บยังไงดีหล่ะ มี 2 วิธีคับ ได้แก่
            - In-line : วิธีนี้คือการที่เราจะเก็บชื่อไฟล์รวมไว้กับ ขนาดของ file entry เลยซึ่งมันทำให้ขนาด file entry ของแต่ละไฟล์มีความไม่เท่ากันเลย แล้วพอมันมีความไม่เท่ากัน กันเกิดขึ้นมันก็อาจจะแบ่งพื้นที่อยากว่าแต่ละไฟล์ควรได้เท่าไร
            - In-Heap : จึงทำให้เกิดวิธีนี้ คือ เราสามารถกำหนดขนาดของ file entry ได้เลยว่าจะเอาเท่าไร จากนั้นในแต่ละ file จะมีพื้นที่ pointer เพื่อชี้ไปยังชื่อของตัวเองในพื้นที่ของ heap 
        > หน้า(20-หน้าสุดท้าย)
            - 1 file : 1 I-node ซึ่งใน i-node จะมีการเก็บ metadata ของไฟล์และนอกจากนี้ แต่ละ disk block ยังมีการเก็บ content อีกด้วย ซึง ส่วนประกอบ หลังจาก i-node จะมี
                Direct node : คือ โหนดที่สามารถชี้ไปหา content นั้นได้เลยทันทีและนอกจากนี้ Direct node ยังมีปริมาณจำกัดอีกด้วย ถ้าหากว่าไฟล์เกิดการโตจนเลยขอบเขตพื้นที่ของ Direct node ก็จะมีอีกตัวนึงมารองรับ ชื่อว่า
                Indirect node : คือ โหนดที่จะทำการชี้ไปหา table ที่ทำการเก็บ Direct Address เอาไว้อีกที ซึ่งระดับของ Indirect จะมี 3 ระดับ ได้แก่
                    - Single indirect block
                    - Double indirect block
                    - Triple indirect block
                    ซึ่งถ้าหากว่าใช้ single เต็มก็ใช้ Double ต่อแล้วก็แบบนี้ไปเรื่อยๆ
            - File System : i-node number + File name
            - Shared File : การจะแชร์ไฟล์ได้นั้นต้องเกิดจากการที่ File ที่ต้องการจะทำงานร่วมกันต้องเห็น i-node ของไฟล์ที่ตนเองต้องการจะทำงานด้วยก่อน ซึ่งถ้าเห็น i-node แล้วไฟล์ดังกล่าวก็จะสามารถเข้าไปใช้ไฟล์นั้นได้เลย โดยถ้าหาก File ที่เข้ามาทำงานด้วยเปลี่ยนแปลงอะไร ไฟล์เจ้าของจะรู้หมดเพราะไฟล์ที่เข้ามาทำงานด้วย จะทำงานผ่าน i-node ทำให้ไฟล์เจ้าของรู้หมดนั่นเอง นอกจากนี้ถ้าหากว่าไฟล์เจ้าของเกิดการลบไฟล์นี้ทิ้งแล้ว ไฟล์ดังกล่าวก็จะต้องไปเป็นของไฟล์ที่ต้องการจะเข้ามาทำงานด้วยไปด้วยปริยาย ซึ่งค่า count จะบ่งบอกว่า ไฟล์นี้มีการใช้อยู่กี่คนนั่นเอง ถ้าอยากให้ค่า count ลดลง ไฟล์ใดไฟล์นึงก็ต้อง remove โดยการลบ i-node ทิ้ง
            - Disk Space Management : มันคือการ Trade-off ระหว่าง ค่า 3 ค่าได้แก่
                1. ยิ่ง Block size มีขนาดใหญ่อัตราการไหลของข้อมูลก็จะเร็วมากขึ้น
                2. ถ้าหาก Block size มีขนาดที่ใหญ่ อัตรการใช้พื้นที่ของ Block size ก็จะใช้น้อยลง เพราะจะส่งผลให้เกิด Internal Fragmentation
            - Track Free block : การ Track block size ที่ว่าง สามารถใช้ structure ดูได้ 2 แบบ คือ 
                - Linked list : โดยแต่ละ Block ของ Linked List จะมีการเก็บ Block อื่นๆที่ยังว่างอยู่ด้วย ถ้า Block นี้ใช้ครบแล้วก็ชี้ไปยัง Block ถัดไปที่ว่าง
                - Bit-Map : 1 bit : 1 disk block
            - Disk Quotas : คือลิมิตของขนาดพื้นที่ของไฟล์ที่ใช้พื้นที่บน Disk ซึ่ง limit นั้นมีการกำหนดไว้ 2 อย่าง คือ
                1. จำนวน Block ที่ใช้ได้
                2. จำนวน i-node ที่สามารถใช้ได้ user ไม่สามารถสร้างไฟล์ได้อย่าง unlimited เพราะยิ่งสร้างไฟล์ก็ยิ่งมีการใช้ i-node ตามจำนวนไฟล์ที่สร้าง
                ** โดย Limit จะมี 2 ระดับ ได้แก่ **
                    1. Soft Limit : คือ Limit ที่ user สามารถหลีกเลี่ยงได้หากเกิน limit แล้ว แต่ระบบก็จะแจ้งเตือนมาเป็นระยะๆ แล้วถ้าหากว่า user ไม่ยอมแก้ไขสักที จะทำให้ไฟล์นั้นโดน terminated ในที่สุด
                    2. Hard Limit : ก็คือเวอร์ชั่น soft limit ที่มีการส่งแจ้งเตือนไปแล้ว แต่ user ไม่ทำอะไรเลยก็ลบไฟล์ทิ้งแม่ง หรืออีกกรณีนึงก็คือ เราเตือนไปแล้วแต่ user ยังสร้างเพิ่มอะ เพิ่มแบบมหาศาล อันนั้นก็ลบ
            - File System Backup : คือการที่เราต้อง Backup ไฟล์จากอันตรายต่างๆ เช่น อาจจะเป็น ภัยพิบัติ หรือเราโง่เผลอกดลบเอง ซึ่งการ Backup file ที่เราจะทำกันมี 2 อย่าง คือ
                1. Full Backup : Backup ทั้งหมดเลย ใช้พื้นที่สูงถ้าทำบ่อยๆ
                2. Increment Backup : Backup แค่ในส่วนที่เปลี่ยนแปลง อันนี้ใช้พื้นที่น้อย ส่วนมากเขาเลยสลับกันทำ เช่น ถ้าอยาก Backup อะไรที่ละนิดละหน่อยก็ใช้อันสอง แต่ถ้าแบบอยากผ่านไปสัก 2 - 3 เดือนแล้ว ก็ใช้อันแรก ประมาณนี้
            - File Consistency : อันนี้ไม่มีอะไรมาก จำไว้เลยมันจะแบ่งเป็น 2 ส่วน คือ 
                - Block In use : ใช้งานอยู่ แสดงค่า 1
                - Free Block : ว่างงับไม่ได้ทำอะไร แสดงค่า 1
                2 ค่านี้จะแสดงสถานะของแต่ละ block number ถ้าหากว่า block number ที่ n มีสถานนะ Block In use = 1 หมายความว่า Free block ต้องเป็น 0 เพราะมันไม่ว่างเลยเป็น 0 จบ 
                ** ทั้ง 2 ส่วน ต้องเป็นเลข 0 กับ 1 เท่านั้นถ้าเป็นเลขอื่นๆ ก็ต้องแก้ให้มันถูก** ดูหน้า(34)
```
...
```
    > หน้า(01-19)
        - ตอนแรกจะกล่าวเกี่ยวกับว่า เออประเภทของ I/O มีอะไรบ้าง ซึ่งเราแบ่งได้ 2 ประเภท คือ 
            1. ติดต่อกับ user
            2. ติดต่อกันเอง
        จากนั้นก็จะเป็นการบอกว่า เนี่ยหลาย I/O อะเขาก็มีการทำงานที่ต่างกันไม่ว่าจะเป็นในเรื่องการส่งข้อมูลหรือการเกิดข้อผิดพลาดอะ ต่าง I/O ก็ต่างมีข้อผิดพลาดที่จำเพาะเจาะจง
        - เวลาที่เกิด Interrupts อะ Interrupts Control จะส่ง Inter มาทาง Interrupts Line ตลอดเพื่อเลี่ยงการส่งใน bus เดี๋ยวมันช้า พอ cpu รัน cycle ของ process นั้นๆจบแล้ว พอ cpu เห็น Interrupts CPU ก็จะส่งข้อความไปบอกว่า เออได้รับ Interrupts แล้วนะ เนี่ยส่งมาใช่ไหมอันนี้ ประมาณนี้
        - ฉะนั้นเขาเลยบอกว่า แล้ว OS หรือว่า CPU ทำยังไงให้ I/O ที่มีลักษณะที่แตกต่างกันขนาดนี้สามารถทำงานร่วมกันได้ ซึ่งมันมีทั้งหมด 3 ประเภท ได้แก่
            - Programmed I/O : อันนี้คือ cpu เป็นคนจัดการทุกอย่างเลยอยู่ด้วยกันตั้งแต่ I/O เริ่มทำงานจนทำงานเสด ซึ่งเมื่อพอเรามาพิจารณาถึงความคุ้มค่าแล้วอะ มันไม่ค่อยคุ้มเลยที่จะต้องให้ cpu มารอ I/O เพราะ I/O ทำงานช้านะ
            - Interrupt-driven I/O : จึงเกิดหลักการนี่แหละที่บอกว่า ถ้างั้น cpu แค่เป็นตัวเริ่มทำงานให้หน่อยแล้วเดี๋ยวที่เหลือทำงานต่อเอง พอจะเสดแล้วเดี๋ยวส่ง interrupt ไป แต่ว่าสุดท้ายแล้วเรื่องการย้ายข้อมูลจาก Mem -> I/O cpu ก็ต้องทำอยู่ดี
            - DMA(Direct Memory Access) I/O : อันนี้คือเหมือนแบบ cpu มอบหมายงานให้ตัวของ DMA Controller เลยส่งว่าต้องทำอะไรบ้าง แล้วที่เหลือให้ DMA ทำจนเสด พอทำเสดหมดแล้วค่อยส่ง Interrupts มาบอก cpu ว่าเออทำเสดแล้วนะ เรียบร้อยรวมถึงการย้ายข้อมูลจาก Mem -> I/O ด้วยที่ DMA ทำเองเลย
    > หน้า(20-22)
        - เราจะกล่าวเกี่ยวกับวิวัฒนาการของ I/O โดยจะสรุปสั้นๆ ก็คือ
            1. cpu มีความสามารถในการทำงานร่วมกับ I/O
            2. cpu ทำงานผ่าน I/O Module ที่ควบคุม I/O อีกที
            3. เกิดการใช้ Interrupts เพื่อให้ cpu ได้ทำ process อื่นๆระหว่างรอ
            4. เกิด DMA ขึ้น
            5. I/O Modules สามารถประมวลผลคำสั่งที่เกียวกับ I/O ได้เองแล้ว
            6.เหมือนข้อ 5 เลยแบบว่า I/O สามารถรันคำสั่งที่เกี่ยวกับ I/O ได้เองบนหน่วยความจำของตนเอง
    > หน้า(21-40)
        - ตอนดูคลิปส่วนนี้ง่วงนิสๆ ซึ่งเหมือนใจความหลักๆ คือ เขาอยากจะแบบว่าใช้ I/O ให้มันแบบมีความเป็นเหมือนกันทางด้านการใช้งานต่างๆ แบบเหมือนจัดประเภท อะ
        - มีพูดเกี่ยวกับ Interrupt Handler แต่วิธีการทำงานก็คล้าย Page fault interrupts
        - Efficiency : มันประมาณแบบว่า ในยามที่ process ส่วนมากต้องไปรอ I/O แล้ว I/O เขาทำงานช้าจนทำให้ CPU Idle แล้วจังวะนี้้ต้องทำอย่างไรดีเพื่อให้เรามี process ที่สามารถทำงานขนาดไปด้วยได้ วิธีก็คือ การ swap process ขึ้นมาเพื่อให้ cpu ไม่ว่าง แต่ว่าการ swap ก็ต้องใช้ I/O เหมือนกันแต่ว่าสุดท้ายแล้วก็จะมีงานทำ cpu ก็จะไม่ว่างเหมือนเดิม
        - I/O software layer : มี 5 ขั้นเรียงจากบนลงล่าง
            1. User-level I/O Software : เรียกใช้ I/O คับจบ แบบ printf อะไรงี้
            2. Device-independent operating system software : เหมือนแบบทำหน้าที่ระบุเกี่ยวกับพวกแบบ ชื่อ สิทธิ์ในการเข้าถึง การใช้ buffer การจองพื้นที่ให้กับ I/O นี้จาก I/O มาเป็น page 
            3. Device drivers : อันนี้คือ เริ่มยุ่งกับ I/O และเริ่มดูพวกแบบสถานะการพร้อมใช้งาน set ค่าอะไรให้เรียบร้อย
            4. Interrupts handlers : อันนี้ก็แบบเตรียมสื่อสารกับ cpu เลอะ
            5. Hardware : อันนี้คือสั่งให้ ตัวของ Hardware ทำงานเลยคับ เช่น คีย์บอร์ด เมาส์ อะไรงี้
        - เราพยายามจะให้ Interface ของแต่ละ Device มีหน้าตาที่เหมือนกันเพื่อช่วยให้ OS ไม่ต้องมีแบบหลาย Interface
        - I/O Buffer คืออะไร ?
            I/O Buffer มี 2 ประเภท ได้แก่ Block and Stream oriented ซึ่งทั้งคู่จะมีในแง่ของการทำ buffer ที่เหมือนกัน คือ 
                - None buffer : user process หากต้องการจะเข้าถึง I/O กูบอกเลยรอโคตรนานกว่าจะย้ายมาแต่ละที เลยต้องม buffer นี่แหละ
                - Single buffer : ช่วยในการเอาค่าจาก I/O มาไว้บน Memory ล่วงหน้า ซึ่งถ้าหากว่ามี user process ต้องการจะเข้าถึงก็สามารถเข้าถึงได้เลยแบบไม่ต้องรอ ไม่ว่าจะเป็นการอ่าน หรือเขียน ทุกอย่างก็จะถูกพักไว้ที่ buffer ก่อนแล้วจึงถูก I/O จัดการทีหลัง
                - Double buffer : คือการที่ว่า หาก user process ต้องการที่จะใช้ I/O มากๆ ก็อาจจะทำให้ต้องมี buffer ขึ้นมาสองอัน ดูหน้า(38) เพื่อตอบสนองต่อ user process
                - Circular buffer : อันนี้คือแบบ ถ้าหากว่า user ต้องการที่จะแบบ ยุ่งกับ I/O อย่างเดียวเลยก็ต้องมีการใช้ buffer แบบ circular นั่นเอง ดูได้จากหน้า(39)
    > หน้า(41-หน้าสุดท้าย)
        - Disk Arm Scheduling Algorithms : คือต้องบอกอย่างงี้ก่อนว่าการที่ Disk จะทำงานได้เนี่ยเขาจำเป็นต้องหมุนจนแบบให้ถึงรอบก่อนจึงจะเริ่มการทำงานได้ ซึ่งตัววัดประสิทธิภาพของ Disk Arm มี 3 ตัวได้แก่
            1. Seek time : เวลาที่หา Track ที่ตัวเองต้องการจะใช้ก็คือ Disk Arm ก็ต้องเลื่อนเข้าเลื่อนออกอะ แล้วยังเป็นตัวบ่งบอกประสิทธิภาพด้วย
            2. Rotation Delay : คือการที่ตอนนี้เราอยู่ถูก Track และแต่แม่งผิด sector ก็ต้องหมุนเป็นวงกลม เพื่อหา sector ที่ต้องการจะใช้ให้ได้
            3. Data transfer time : เวลาในการเขียนนี่แหละ
            ซึ่งเราจะมี Policies ในการเขียนลง Disk เราเรียกว่า Disk Scheduling Policies ซึ่งมีหลักๆ 4 policies ได้แก่
                1. FIFO : เริ่มจาก current track แล้วพุ่งตรงไปยัง Disk request เลย หน้า(46)
                2. SSTF(Shortest Seek Time First) :เริ่มจาก current track คับต่อจากนั้นเราจะพุ่งตรงไปหาค่า Disk request ที่อยู่ใกล้กับ current trak มากที่สุดแล้วไปทางนั้นแหละ หน้า(48)
                3. SCAN : เริ่มจาก current track ต่อจากนั้นเราจะไปหา Disk request ที่มีค่ามากกว่า current track แล้วมุ่งตรงจนสุดส้นตีนน แล้วค่อย reverse กลับมาใหม่จนถึงค่าที่น้อยสุด หน้า(49)
                4. C-SCAN : เหมือน SCAN ทุกอย่างต่างกันแค่ตรง reverse ที่เราจะ reverse จากค่าที่ต่ำสุด มาค่ากลาง ถ้างงดูหน้า(50) เลยน้อนๆ
