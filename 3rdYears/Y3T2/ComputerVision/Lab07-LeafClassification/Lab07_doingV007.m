...
for i = 1:numTrain
    ...
    if ~isempty(stats_train)
        ...
        [~, maxIdx_train] = max([stats_train.Area]);
        stats_train = stats_train(maxIdx_train);
        
        featureVectorTrain = [stats_train.Perimeter, stats_train.Area, stats_train.Eccentricity, ...
...
for i = 1:numTest
    ...
    if ~isempty(stats_test)
        [~, maxIdx_train] = max([stats_test.Area]);
        stats_test = stats_test(maxIdx_train);

        featureVectorTest = [stats_test.Perimeter, stats_test.Area, stats_test.Eccentricity, ...
    ...
    feature_vector_test(i, :) = featureVectorTest;
end
