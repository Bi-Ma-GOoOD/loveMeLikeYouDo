...
# Toon shading with Phong reflection model
class ToonPhong(Material):
    def __init__(self, cAlbedo, kd, ks, fAlpha, toon_levels=4, spec_threshold=0.5) -> None:
        super().__init__()
        self.color_albedo = rtu.Color(cAlbedo.r(), cAlbedo.g(), cAlbedo.b())
        self.kd = kd
        self.ks = ks
        self.alpha = fAlpha
        self.toon_levels = toon_levels    # Number of discrete levels for toon shading
        self.spec_threshold = spec_threshold  # Threshold for specular highlight
    ...
    def scattering(self, rRayIn, hHinfo):
        ...
        reflected_direction = uvw.local(rtu.Vec3.random_cosine_hemisphere_on_z())
        reflected_ray = rtr.Ray(hHinfo.getP(), reflected_direction, rRayIn.getTime())
        toon_color = self.BRDF(rRayIn, reflected_ray, hHinfo)

        return rtu.Scatterinfo(reflected_ray, toon_color)

    def BRDF(self, rView, rLight, hHinfo):
        # Calculate base diffuse intensity
        light_dir = rtu.Vec3.unit_vector(rLight.getDirection())
        normal = rtu.Vec3.unit_vector(hHinfo.getNormal())
        L_dot_N = max(0, rtu.Vec3.dot_product(light_dir, normal))
        
        # Quantize the diffuse shading based on toon levels
        step = 1.0 / self.toon_levels
        quantized_L_dot_N = math.floor(L_dot_N / step) * step
        
        # Apply quantized diffuse color
        diff_color = rtu.Color(self.color_albedo.r(), self.color_albedo.g(), self.color_albedo.b()) * self.kd * quantized_L_dot_N

        # Calculate specular component with threshold
        perfect_reflection = rtu.Vec3.unit_vector(reflect(-rLight.getDirection(), hHinfo.getNormal()))
        viewing_vector = rtu.Vec3.unit_vector(-rView.getDirection())
        R_dot_V = max(0, rtu.Vec3.dot_product(perfect_reflection, viewing_vector))

        # Apply a threshold to control whether the specular highlight appears
        spec_color = rtu.Color(0, 0, 0)  # Default to no specular
        if R_dot_V > self.spec_threshold:
            spec_color = rtu.Color(self.color_albedo.r(), self.color_albedo.g(), self.color_albedo.b()) * self.ks

        # Return combined toon-shaded color (diffuse + specular)
        return diff_color + spec_color
