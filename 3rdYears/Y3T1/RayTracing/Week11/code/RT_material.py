...
# fr = kd + ks*(R.V)^roughness
# Kd = Material    
class TexturePhong(Material):
    def __init__(self, texture_or_color, kd, ks, alpha) -> None:
        super().__init__()
        
        # Initialize texture or color
        if isinstance(texture_or_color, rtu.Color):
            self.color_albedo = rtt.SolidColor(texture_or_color)
        else:
            self.color_albedo = texture_or_color  # Use provided texture
        
        # Initialize Phong reflection parameters
        self.kd = kd  # Diffuse strength
        self.ks = ks  # Specular strength
        self.alpha = alpha  # Shininess

    def scattering(self, rRayIn, hHinfo):
        # Create the ONB for local coordinate system based on the normal
        uvw = rtu.ONB()
        uvw.build_from_w(hHinfo.getNormal())

        # Generate the scattered direction
        scattered_direction = uvw.local(rtu.Vec3.random_cosine_hemisphere_on_z())
        scattered_ray = rtr.Ray(hHinfo.getP(), scattered_direction, rRayIn.getTime())

        # Calculate the BRDF value (diffuse and specular)
        texture_phong_color = self.BRDF(rRayIn, scattered_ray, hHinfo)

        # Return scatter information
        return rtu.Scatterinfo(scattered_ray, texture_phong_color)

    def BRDF(self, rView, rLight, hHinfo):
        # Calculate the diffuse color from texture (modulated by kd)
        tex_color = self.color_albedo.tex_value(hHinfo.u, hHinfo.v, hHinfo.point)
        diff_color = tex_color * self.kd * (1.0 / math.pi)
        ...
        spec_color = tex_color * self.ks * math.pow(R_dot_V, self.alpha)

        # Return the combined diffuse + specular result
        return diff_color + spec_color
