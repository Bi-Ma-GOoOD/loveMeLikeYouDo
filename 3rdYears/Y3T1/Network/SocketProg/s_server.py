...
def send_file(client_socket, file_path, new_file_name):
    try:
        print(f"BiMaGOoOD : Confirm status (200)")
        print(f"BiMaGOoOD : Ready to send file from server to {client_socket.address[1]}")
        ...
        client_socket.send(f"BiMaGOoOD : Error to send file.\n\tCause : {e}".encode('utf-8'))

def handle_client_connection(client_socket, addr):
    try:
        while True:
            song_list = "\n".join([f"{key}: {value}" for key, value in SONGS.items()])
            # Send song's list to client
                ...
                if song_id.isdigit() and int(song_id) in SONGS:
                    ...
                    if lyrics_choice == "P":
                        ...
                        try:
                            ...
                            client_socket.send(b'EOF')
                            print(f"BiMaGOoOD : Sent EOF marker to {addr[1]} for transfer file.") 
                        except Exception as e:
