...
def gl_init_models():
    ...
    df = pd.read_csv("D:\\AB-BiMaGOoOD\\Tob-taun\\3rdYears\\Y3T1\\ICG\\Week05\\bunny.tri", sep='\s+', 
    ...
    for i in range(n_vertices) :
        ...
        VdotR = max(0, np.power(np.dot(V, R), n))
        # V = eye_pos - positions[i]
        # R = 2 * np.dot(N, L) * N - L
        # Ia = 0.1
        # Id = 0.5 * max(0, np.dot(N, L))
        # Is = 0.4 * max(0, np.dot(R, V)) ** 10
        # colors[i] = colors[i] * (Ia + Id) + Is
