...
def draw_gui():
    ...
    #Shinniness
    _, Shininess = imgui.slider_int("Shininess", Shininess, 1, 100)
    #Enable Specular
...
def callMat():
    ...
    for i in range(n_vertices) :
        ...
        if two_side :
            if (np.dot(N, L) > 0) :
                Kd = np.array(mat_d_front) * max(np.dot(N, L), 0) * I
            else :
                Kd = np.array(mat_d_back) * max(np.dot(-N, L), 0) * I
        else :
            Kd = np.array(mat_d_front) * max(np.dot(N, L), 0) * I
        diffuse = np.array(Kd) * I * NdotL
